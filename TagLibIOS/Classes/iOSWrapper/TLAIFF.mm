...
//  Pods
//
//  Created by BTRLabs on 10/30/19.
//
...
- (instancetype)initWithFileAtPath:(NSString *)path {
    if (self = [super init]) {
        ...
        if (_path != nil) {
            ...
            file = (TagLib::RIFF::AIFF::File *) fileRef.file();
        }
    }
    return self;
}
...
- (NSDictionary *)audioProperties {
    AudioProperties *props = self->file->audioProperties();
    NSMutableDictionary *dict = [[NSMutableDictionary alloc] init];
    [dict setObject:@"AIFF" forKey:@"FILETYPE"];
...
- (NSData *)frontCoverPicture {
    TagLib::ID3v2::FrameList frameList = file->tag()->frameListMap()["APIC"];
